...
//    }
    ...
    @Override
    public Customer getCustomerCode(String customerCode) {
        String sql = "SELECT customer_code FROM tbl_customer WHERE customer_code = ?";
    ...
    public int getQuantityCustomer() {
        String sql = "SELECT COUNT(*) as quantity_customer FROM tbl_customer";
        int quantity = 0;
        try {
            ps = conn.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                quantity = rs.getInt("quantity_customer");
            }
